...
<template>
  <cytomine-modal :active="active" :title="$t('button-add-image-from-upload')" @close="$emit('update:active', false)">
...
import {Cytomine, AbstractImage, AttachedFile, Configuration, Description, ImageInstance, StorageAccessCollection, TagCollection, TagDomainAssociation, UploadedFile, StorageUserCollection} from 'cytomine-client';
...
export default {
  ...
  computed: {
    ...
    uploadable(){
      return this.selectedStorage != null && this.selectedImage != null && this.userCanWriteInStorage;
    },
    userCanWriteInStorage() {
      console.log(this.selectedStorageUsers===undefined);
      console.log(this.selectedStorageUsers);
  ...
  methods: {
    ...
    async fetchStorages() {
      try {
        console.log((await StorageAccessCollection.fetchAll()).array);

      ...
      >
        <template #default="{row: image}">
          ...
          <b-table-column :field="score.name" :label="score.name" centered sortable width="100" v-for="score in scores" :key="score.id">
            <b-select size="is-small" v-model="image[score.id]" @input="event => changeValue(event, score, image)">
              <option :value="null">
                {{$t('no-key-selected')}}
              </option>
...
import {GenericCollection, ConsensusScore} from 'cytomine-client';

// store options to use with store helpers to target projects/currentProject/listImages module
const storeOptions = {rootModuleProp: 'storeModule'};

export default {
  name: 'project-consensus-scores-group-by-image',
  components: {
    ImageName,
    ImageDetails,
    CytomineTable,
  ...
  computed: {
    ...
    statsCollection() {
      let collection = new GenericCollection({
        path: `project/${this.project.id}/consensus-score/stats-group-by-image.json`
      });
  ...
  methods: {
    async changeValue(event, score, image) {
      if (event!=null) {
        await new ConsensusScore({imageInstance: image.id, score: score.id, scoreValue: event}).save();
      }
      else {
        await new ConsensusScore({imageInstance: image.id, score: score.id, id: 0}).delete(); // hack: cannot delete if id is null
      }
      // await this.loadConsensusScore();
    },
    printScoreValue(scoreValueId, score) {
      console.log(scoreValueId, score);
      let val = score.values.find(value => value.id === scoreValueId);
      if (val!=null) {
        return val.value;
      }
      return null;

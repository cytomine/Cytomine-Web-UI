...
VERSION_NUMBER=$(<"$file")
echo "$1"
docker build --no-cache --rm -f scripts/docker/web-ui/Dockerfile --build-arg CURRENT_BRANCH=$1 -t  repository.cytom.in:5004/web_ui-$CUSTOMER:v$VERSION_NUMBER ./scripts/docker/web-ui

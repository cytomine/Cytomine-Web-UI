...
# - NGINX_VERSION: [REQUIRED] an cytomine specific image version to append to rabbitmq version
...
if [ -z "$WEB_UI_VERSION" ]; then
  echo >&2 "$ME: ERROR: WEB_UI_VERSION not provided"
  exit 1
fi
...
docker build \
  ...
  -t "$DOCKER_NAMESPACE/web_ui:$WEB_UI_VERSION" .
